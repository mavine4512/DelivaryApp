...
export default function ItemInfo({route, navigation}) {
  ...
  useEffect(() => {
    ...
    async function fetchMyAPI() {
      let response = await fetch(
        'https://www.omdbapi.com/?i=' + item.imdbID + '&apikey=b001395c',
      );
  ...
  const theWriter = (Writers) => {
    ...
    return WritersArray.map((y) => (
      <Text style={styles.textActors} key={y}>
        {y}
      </Text>
    ));
  };
  ...
  const theActors = (actors) => {
    ...
    return actorsArray.map((x) => (
      <Text style={styles.textActors} key={x}>
        {x}
      </Text>
    ));
  };
              ...
              <View style={styles.ItemInfo}>
                ...
                <View>
                  <Text style={styles.text}>
                    Production : {moviesDetails.Production}
                  </Text>
                  <View
                    style={{
                      flexDirection: 'row',
                  ...
                  <Text style={styles.text}>
                    Awards : {moviesDetails.Awards}
                  </Text>
                  <Text style={styles.text}>
                    Language : {moviesDetails.Language}
                  </Text>
                  <Text style={styles.text}>Genre : {moviesDetails.Genre}</Text>
                  <Text style={styles.text}>
                    Released : {moviesDetails.Released}
                  </Text>
                  <Text style={styles.text}>
                  ...
                  <Text style={styles.text}>
                    Metascore : {moviesDetails.Metascore}
                  </Text>
